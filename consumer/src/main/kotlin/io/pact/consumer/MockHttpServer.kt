...
import io.pact.core.matchers.FullRequestMatch
import io.pact.core.matchers.PartialRequestMatch
import io.pact.core.matchers.RequestMatching
import io.pact.core.matchers.generators.ArrayContainsJsonGenerator
import io.pact.core.model.DefaultPactWriter
...
import io.pact.core.model.queryStringToMap
import io.pact.core.support.CustomServiceUnavailableRetryStrategy
import io.pact.core.support.unwrap
...
abstract class BaseMockServer(val pact: RequestResponsePact, val config: MockProviderConfig) : AbstractBaseMockServer() {
  ...
  private fun invalidResponse(request: Request): Response {
    ...
    return Response(500, mutableMapOf("Access-Control-Allow-Origin" to listOf("*"), "Content-Type" to listOf("application/json"),
      ...
      io.pact.core.model.ContentType.JSON))
  }
...
abstract class BaseJdkMockServer(
    ...
    if (exchange.requestMethod == "OPTIONS" && exchange.requestHeaders.containsKey("X-PACT-BOOTCHECK")) {
      ...
      try {
        ...
        pactResponseToHttpExchange(Response(500, mutableMapOf("Content-Type" to listOf("application/json")),
          OptionalBody.body("{\"error\": ${e.message}}".toByteArray(), io.pact.core.model.ContentType.JSON)), exchange)
      }
    }
  }
  ...
  private fun contentType(headers: Headers): io.pact.core.model.ContentType {
    ...
    return if (contentType != null && contentType.value.isNotEmpty()) {
      io.pact.core.model.ContentType(contentType.value.first())
    } else {
      io.pact.core.model.ContentType.JSON
    }
  }
