...
import io.pact.core.support.Json.extractFromJson
import io.pact.core.support.Utils.extractFromMap
...
class MessagePact @JvmOverloads constructor (
  ...
  fun mergePacts(pact: Map<String, Any>, pactFile: File): Map<String, Any> {
    ...
    val messages = (newPact["messages"] as List<Map<String, Any>>) +
      (json["messages"].asArray()?.values?.map { Json.toMap(it) })?.distinctBy { it["description"] }
