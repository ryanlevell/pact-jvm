...
class ReporterManagerSpec extends Specification {
  ...
  def 'can create an instance using fully qualified name'() {
    expect:
    ReporterManager.createReporter('io.pact.provider.reporters.AnsiConsoleReporter', '/tmp/' as File) != null
  }

  def 'should throw an exception for none reporter classes'() {
    when:
      ReporterManager.createReporter('io.pact.provider.ConsumerInfo', '/tmp/' as File)
    then:
